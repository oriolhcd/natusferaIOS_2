        ...
        if (indexPath.item == 0) {
            ...
            if ([self.observation.geoprivacy isEqualToString:@"obscured"]) {
                cell.geoprivacyLabel.attributedText = ({
                    FAKIcon *obscured = [FAKNatusfera icnLocationObscuredIconWithSize:24];
                ...
                cell.geoprivacyLabel.attributedText = ({
                    FAKIcon *private = [FAKNatusfera icnLocationPrivateIconWithSize:24];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section < 2) {
        ...
        if (self.observation.dataQuality == ObsDataQualityNone) {
            NSURL *dataQualityURL = [NSURL URLWithString:@"http://inaturalist.gbif.es/pages/help#quality"];
            if (dataQualityURL) {
