...
@implementation NatusferaAppDelegate
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        if (![[NSUserDefaults standardUserDefaults] stringForKey:INatUsernamePrefKey]) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [((NatusferaAppDelegate *)[UIApplication sharedApplication].delegate) showInitialSignupUI];
            });
        } else {
            dispatch_async(dispatch_get_main_queue(), ^{
                [((NatusferaAppDelegate *)[UIApplication sharedApplication].delegate) showMainUI];
...
- (RKManagedObjectStore *)inatObjectStore {
    if (!_inatObjectStore) {
        _inatObjectStore = [RKManagedObjectStore objectStoreWithStoreFilename:@"natusfera.sqlite"
...
- (void)configureRestKit
{
    ...
    //NSString *userAgent = [NSString stringWithFormat:@"Natusfera/%@ (iOS %@ %@ %@)", //M.Lujano:10/06/2016
...
- (NSManagedObjectModel *)getManagedObjectModel
{
    ...
    NSString *storePath = [docDir stringByAppendingPathComponent:@"natusfera.sqlite"];
